...
#pragma omp barrier
        ...
        while (1)
        {
                ...
                messageBuffers[threadId].head != messageBuffers[threadId].tail)
            {
                ...
                byte procNodeAddr = (msg.address >> 4) & 0xF;
                byte memBlockAddr = msg.address & 0xF;
                ...
                case READ_REQUEST:
                    ...
                    if (node.directory[memBlockAddr].state == U)
                    {
                        ...
                        msgReply.dirState = EM; // Set directory state for receiver
                    ...
                    else if (node.directory[memBlockAddr].state == S)
                    {
                        ...
                        msgReply.address = msg.address;
                        msgReply.value = node.memory[memBlockAddr];
                        msgReply.dirState = S; // Set directory state for receiver
                        sendMessage(msg.sender, msgReply);
                    }
                ...
                case WRITEBACK_INT:
                    ...
                    for (int i = 0; i < CACHE_SIZE; i++)
                    {
                        if (node.cache[i].address == msg.address &&
                            (node.cache[i].state == MODIFIED || node.cache[i].state == EXCLUSIVE))
                        {
                            msgReply.type = FLUSH;
                            msgReply.sender = threadId;
                            msgReply.address = msg.address;
                ...
                case REPLY_ID:
                    ...
                    for (int i = 0; i < CACHE_SIZE; i++)
                    {
                        if (node.cache[i].address == msg.address && node.cache[i].state == SHARED)
                        {
                            node.cache[i].value = instr.value; // Update with instr.value
                ...
                case INV:
                    ...
                    for (int i = 0; i < CACHE_SIZE; i++)
                    {
                        if (node.cache[i].address == msg.address && node.cache[i].state != INVALID)
                        {
                ...
                case REPLY_WR:
                    ...
                    node.cache[cacheIndex].address = msg.address;
                    node.cache[cacheIndex].value = msg.value; // Use value from message first
                ...
                case FLUSH_INVACK:
                    ...
                    else
                    {
                        ...
                        node.cache[cacheIndex].address = msg.address;
                        // Use the current instruction value if it matches this address
                        if (instructionIdx >= 0 && instructionIdx < node.instructionCount &&
                            node.instructions[instructionIdx].address == msg.address &&
                            node.instructions[instructionIdx].type == 'W')
                        {
            ...
            // instruction address
            byte procNodeAddr = (instr.address >> 4) & 0xF;
            byte memBlockAddr = instr.address & 0xF;
            byte cacheIndex = memBlockAddr % CACHE_SIZE;
            ...
            if (instr.type == 'R')
            {
                ...
                for (int i = 0; i < CACHE_SIZE; i++)
                {
                    if (node.cache[i].address == instr.address && node.cache[i].state != INVALID)
                    {
                        cacheHit = 1;
                        break;
                    }
            ...
            else
            {
                ...
                if (cacheHit)
                {
                    if (node.cache[cacheLineIdx].state == MODIFIED ||
                        node.cache[cacheLineIdx].state == EXCLUSIVE)
                    {
                        node.cache[cacheLineIdx].value = instr.value;
                        node.cache[cacheLineIdx].state = MODIFIED;
                    }
                    else if (node.cache[cacheLineIdx].state == SHARED)
                    {
                        message upgradeReq;
                ...
                else
                {
                    ...
                    writeReq.value = instr.value; // Include instr.value in the request
...
void handleCacheReplacement(int sender, cacheLine oldCacheLine)
{
    ...
    byte memBlockAddr = oldCacheLine.address & 0xF;
